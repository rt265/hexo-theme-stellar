...
- 上块显式分为三行：第一行左侧面包屑、右侧 AI 标签；第二行日期信息靠左；第三行字数与预计阅读时长靠左（阅读时长从第二行右侧移到独立行，简化窄屏响应式布局）；
...
<div class="top bread-nav footnote">
  <div class="top-row">
    ...
    <div class="right">…AI 标签（仅在非空时渲染）…</div>
  </div>
  <div class="meta-row">
    …dateinfo（#post-meta），靠左…
  </div>
  <div class="reading-row">
    …字数与预计阅读时长，靠左（仅在非空时渲染）…
  </div>
</div>
```
...
- 模板：`layout/_partial/main/navbar/article_banner.ejs`
  ...
  - AI 标签拼入 `rightHtml`，非空才渲染 `.right`；阅读时长独立为 `.reading-row`，非空才渲染；
  - `#breadcrumb` id 移到 `.left` 上。
- 样式：
  - `source/css/_components/partial/article-banner.styl`：`.content .top` 改为纵向多行布局（`flex-direction: column`、`align-items: flex-start`），新增 `.top-row`（`space-between`、`width: 100%`）、`.top-row .right`（`flex-shrink: 0`、`white-space: nowrap`）、`.meta-row`（`width: 100%`）、`.reading-row`（`width: 100%`，阅读时长保留 `opacity: .8`）；
...
- 页面矩阵 DOM 核对：tech 文章（含 `banner_info.color`，行 1 无右侧）、story 文章（行 1 右侧 AI 标签、行 3 阅读时长）、about（story + banner_info 头像，底部不居中）、wiki 页（行 2 仅更新日期）、作者归档（无 `.top`、底部原样）、普通 page；
