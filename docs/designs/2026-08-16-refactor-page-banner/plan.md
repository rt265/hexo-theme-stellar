...
1. 重构 `layout/_partial/main/navbar/article_banner.ejs`：上块改为 `.top-row`（`.left#breadcrumb` + `.right`）+ `.meta-row` + `.reading-row`；`breadcrumb: false` 时不再输出空 `.top`。
2. 更新 `source/css/_components/partial/article-banner.styl`：`.top` 纵向多行布局 + 新增 `.top-row` / `.right` / `.meta-row` / `.reading-row` 规则。
