  ...
  if (api) {
    ...
    for (let item of (links[args.group] || [])) {
      if (item?.url) {
        ...
        el += `<img class="lazy" alt="Poster" data-src="${item.cover || item.icon || item.avatar || ctx.theme.config.default.cover}" onerror="javascript:this.removeAttribute(&quot;data-src&quot;);this.src=&quot;${ctx.theme.config.default.cover}&quot;;"/>`
